...
class imdb:
	...
	def __init__(self):
		self.name = self.episode = self.about = self.rating = self.directors = self.stars = ""
		self.more = []
	...
	def find_about(self,film):
		self.about = film.find_all('p',{'class':'text-muted'})[0].text.replace('\n',' ').strip()
	...
	def find_director_stars(self,film):
		...
		if(len(stars_list) == 2):
			...
			if(stars_list[0].split(':')[0].strip() == 'Director'):
				self.directors = stars_list[0].split(':')[1].strip()
			elif(stars_list[0].split(':')[0].strip() == 'Stars' or stars_list[0].split(':')[0].strip() == 'Star'):
	...
	def display(self):
		...
		if(len(self.more) == 2):
			print("Read more : " + self.url + self.more[0] + "  or  " + self.url + self.more[1])
		if(len(self.more) == 1):
...
def main():
	...
	while(flag):
		arr = []
		url = url + "?title=" + keyword + "&user_rating=" + minimum + "," + maximum + "&start=" + str(page_start) + "&ref_=adv_nxt"
		r = requests.get(url)
		...
		if(len(progress) == 4):
			...
			if(page_start > last):
				...
				c = ''
				print("Type 'q' and enter to quit, another key and enter to see next set of entries : ", end="")
				while(c == ''):
					c = input()
				if(c == 'q' or c == 'Q'):
					flag = 0
